...
savedir = './out-nn-full/%s-inv-predict' % loadas_pre
...
model = nn.NNFullModelForPints(trained_nn_model, stim_nodes, stim_pos, shape,
        transform_x=None,
    ...
    try:
        ...
        if debug:
            ...
            for i, j_stim in enumerate(stim_nodes):
                ...
                # Re try
                predict_x = [np.append([i, j_stim],
                                       logtransform_x.transform(input_value))
            ...
            plt.savefig('invabc-nn-full-debug')
